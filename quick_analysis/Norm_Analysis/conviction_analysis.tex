...
Nine \textbf{node selection strategies} are compared at each mask level $k$:
\medskip
\begin{enumerate}
    ...
    \item \textbf{deg\_low} --- bottom-$k$\% nodes by degree
    \item \textbf{deg\_matched\_high} --- $k$\% nodes matching the degree distribution of the \emph{high}-conviction group
    \item \textbf{deg\_matched\_low} --- $k$\% nodes matching the degree distribution of the \emph{low}-conviction group
    \item \textbf{conf\_high} --- top-$k$\% nodes by model prediction confidence $\max_c \hat{p}(y=c)$
    \item \textbf{conf\_low} --- bottom-$k$\% nodes by model prediction confidence
    \item \textbf{random} --- $k$\% uniformly random nodes
\end{enumerate}
\medskip
\small The degree-matched samples are built by matching each target node to a candidate of the same degree (or nearest available), drawn from a pre-shuffled pool.
\end{frame}
